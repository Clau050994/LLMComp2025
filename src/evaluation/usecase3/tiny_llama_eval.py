#!/usr/bin/env python3
"""
Enhanced evaluation script for TinyLLaMA model with additional metrics:
...
def load_model_and_tokenizer(model_path):
    ...
    try:
        ...
        try:
            ...
            # Fall back to TinyLLaMA tokenizer
            print("Using default TinyLLaMA tokenizer...")
...
def benchmark_comparison(metrics):
    ...
    benchmarks = {
        ...
        "TinyLLaMA-1.1B (Generic)": 0.77,  # Approximate baseline for similar sized models
        "DistilBERT on GLUE": 0.825,
        "BERT-base on GLUE": 0.849,
    }
    
    print(f"Our TinyLLaMA model F1: {metrics['f1_weighted']:.4f}")
    for benchmark, score in benchmarks.items():
        print(f"{benchmark}: {score:.4f} ({(metrics['f1_weighted']-score)*100:+.1f}% diff)")


def main():
    parser = argparse.ArgumentParser(description="Evaluate TinyLLaMA model on test datasets")
    ...
    # Print metrics
    print("\nEvaluation Results for TinyLLaMA:")
    ...
    cm_path = os.path.join(args.output_dir, "tinyllama_confusion_matrix.png")
    ...
    results_path = os.path.join(args.output_dir, "tinyllama_evaluation_metrics.csv")
