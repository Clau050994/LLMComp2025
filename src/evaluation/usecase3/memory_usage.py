...
# === Device Powers in Watts ===
DEVICE_POWER_GPU = 250
DEVICE_POWER_CPU = 40
DEVICE_POWER_LAPTOP = 15
DEVICE_POWER_PHONE = 3
...
df["Energy per Query (J) [GPU]"] = df[latency_col].astype(float) / 1000 * DEVICE_POWER_GPU
df["Energy per Query (J) [CPU]"] = df[latency_col].astype(float) / 1000 * DEVICE_POWER_CPU
df["Energy per Query (J) [Laptop]"] = df[latency_col].astype(float) / 1000 * DEVICE_POWER_LAPTOP
df["Energy per Query (J) [Phone]"] = df[latency_col].astype(float) / 1000 * DEVICE_POWER_PHONE

# === Save to CSV ===
output_path = "/aul/homes/cvaro009/Desktop/LLMComp2025/src/evaluation/usecase3/plots_ONNX/energy_usage_summary.csv"
df.to_csv(output_path, index=False)
print(f"✅ Energy summary for all devices saved to {output_path}")
...
plt.legend()
plt.tight_layout()
plt.savefig(output_path.replace(".csv", "_energy_barplot.png"))
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig(output_path.replace(".csv", "_latency_vs_energy_scatter_ALLDEVICES.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(output_path.replace(".csv", "_modelsize_vs_energy_ALLDEVICES.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(output_path.replace(".csv", "_peakmem_vs_energy_ALLDEVICES.png"))
plt.close()

print("✅ All plots with all devices saved to the same directory.")
