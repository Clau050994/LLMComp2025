#!/usr/bin/env python3
"""
Enhanced evaluation script for baseline model with additional metrics:
...
# --------- SET YOUR PATHS HERE ---------
MODEL_PATH = "/disk/diamond-scratch/cvaro009/data/phi3_risk_classification_qlora"  # Your baseline model path
TEST_FILE = "/aul/homes/cvaro009/Desktop/LLMComp2025/data/processed/unified/unified_test.csv"
OUTPUT_DIR = "/aul/homes/cvaro009/Desktop/LLMComp2025/baseline_evaluation_results"  # Baseline results
# ---------------------------------------
...
def load_model_and_tokenizer(model_path):
    ...
    # Handle missing pad token for baseline models
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    
    # Load model with correct number of labels (3 for your risk classification)
    model = AutoModelForSequenceClassification.from_pretrained(
        model_path, 
        num_labels=3,  # Explicitly set to 3 classes
        ignore_mismatched_sizes=True  # This helps with size mismatches
    )
    
    model_size_mb = sum(p.numel() * p.element_size() for p in model.parameters()) / (1024 * 1024)
    print(f"Baseline model loaded: {model_size_mb:.2f} MB")
...
def evaluate_model(model, eval_dataset, device=None):
    ...
    # Standard trainer for baseline model
    trainer = Trainer(
        model=model,
        args=None,
...
def plot_confusion_matrix(cm, save_path=None, model_name="Baseline Model"):
    ...
    plt.close()  # Close to prevent display in terminal
...
def benchmark_comparison(metrics, model_name="Baseline Model"):
    print(f"\n{model_name} Performance in Context:")
    benchmarks = {
        "Random Baseline (3 classes)": 0.33,
        "Majority Class Baseline": 0.39,
        "Simple Logistic Regression": 0.65,
        "BERT-base on GLUE (avg)": 0.849,
        "DistilBERT on GLUE (avg)": 0.824,
        "ALBERT-base on GLUE (avg)": 0.833,
        "GPT-3.5 fine-tuned": 0.88,
        "SOTA Models on GLUE (2023)": 0.92
    }
    
    print(f"Our {model_name} F1: {metrics['f1_weighted']:.4f}")
    for benchmark, score in benchmarks.items():
        diff = (metrics['f1_weighted'] - score) * 100
        print(f"{benchmark}: {score:.4f} ({diff:+.1f}% diff)")
...
def main():
    ...
    plot_confusion_matrix(metrics['confusion_matrix'], save_path=cm_path, model_name="Baseline Model")
    ...
    benchmark_comparison(metrics, "Baseline Model")
    
    # Save results
    results_path = os.path.join(OUTPUT_DIR, "evaluation_metrics_baseline.csv")
    metrics_df = pd.DataFrame({k: [v] if not isinstance(v, np.ndarray) else [str(v)] 
                              for k, v in metrics.items()})
    metrics_df.to_csv(results_path, index=False)
    print(f"\nDetailed metrics saved to {results_path}")
    print(f"All baseline results saved in: {OUTPUT_DIR}")
