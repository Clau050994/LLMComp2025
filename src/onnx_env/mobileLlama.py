...
def export_model_to_onnx(model_dir, onnx_dir, model_name="mobilellama"):
    """Export a Hugging Face MobileLlama model to ONNX format with validation."""
    ...
    print("Loading MobileLlama model and tokenizer...")
    try:
    ...
    # Prepare dummy input - shorter sequence for LLaMA models to avoid cache issues
    dummy_text = [
    ...
    # Use shorter sequence length to avoid cache issues
    max_length = 1024 
    inputs = tokenizer(
    ...
    print("Exporting MobileLlama model to ONNX...")
    try:
        torch.onnx.export(
            wrapped_model,
            tuple(inputs_list),
...
if __name__ == "__main__":
    # Paths
    model_dir = "/disk/diamond-scratch/cvaro009/data/mobilellama"
    onnx_dir = "/disk/diamond-scratch/cvaro009/data/onnx_models/mobilellama_onnx"
    
    # Export the model
    success = export_model_to_onnx(model_dir, onnx_dir, "mobilellama")
    
    if success:
        print("\n🎉 MobileLlama export successful!")
    else:
        print("\n❌ MobileLlama export failed!")